...
export default function ChatMessage({ sender, text, isUser, avatar }: ChatMessageProps) {
  return (
    <div className={`flex ${isUser ? "flex-row-reverse" : "flex-row"} items-start gap-3`}>
      ...
      <div className={`flex flex-col ${isUser ? "items-end" : "items-start"} max-w-[70%]`}>
        {/* Sender name */}
        <span className={`text-lg text-purple-800 mb-1 ${isUser ? "text-right" : "text-left"}`}>{sender}</span>
